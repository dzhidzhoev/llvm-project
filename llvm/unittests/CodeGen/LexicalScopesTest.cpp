...
TEST_F(LexicalScopesTest, TestRepeatingSubprogram) {
  std::unique_ptr<MachineFunction> MF2 = createMachineFunction(Ctx, Mod, "Test.1");
  MF2->getFunction().setSubprogram(OurFunc);

  std::unique_ptr<MachineFunction> FooMF = createMachineFunction(Ctx, Mod, "Foo");
  ...
  auto OurSubT = DIB.createSubroutineType(DIB.getOrCreateTypeArray({}));
  DISubprogram *FooFunc = DIB.createFunction(OurCU, "Foo", "", OurFile, 1, OurSubT, 1, DINode::FlagZero, DISubprogram::SPFlagDefinition);
  FooMF->getFunction().setSubprogram(FooFunc);
  DIB.finalize();

  BuildMI(*FooMBB, FooMBB->end(), DILocation::get(Ctx, 10, 20, FooFunc), BeanInst);
