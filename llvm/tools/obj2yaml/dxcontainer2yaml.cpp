...
dumpDXContainer(MemoryBufferRef Source) {
  ...
  for (const auto P : Container) {
    ...
    case dxbc::PartType::ILDN: {
      ...
      assert(DebugName && "Since we are iterating and found a ILDN part, this should never not have a value");
      NewPart.DebugName = DXContainerYAML::DebugName{DebugName->first.Flags, DebugName->first.NameLength, DebugName->second.str()};
      break;
    }
