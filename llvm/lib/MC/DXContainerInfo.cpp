...
void SourceInfoWriter::computeEntries() {
  IsFilled = true;
...
void SourceInfoWriter::recomputeAfterCompression(uint32_t CompressedSize) {
  BaseData.Contents.computeFinalSize(CompressedSize);
  BaseData.compute();
}

void SourceInfoWriter::finalize() {
...
void SourceInfoWriter::write(raw_ostream &OS) const {
  assert(IsFinalized &&
