...
static ModuleMetadataInfo collectMetadataInfo(Module &M) {
  ...
  if (ContentsNode && ArgsNode) {
    ...
    MMDAI.SourceInfo->setCompressionType(compression::zlib::isAvailable() ? dxbc::SourceInfo::Contents::CompressionType::Zlib : dxbc::SourceInfo::Contents::CompressionType::None);
    ...
    for (Metadata *ArgNode : ArgsNode->operands())
      MMDAI.SourceInfo->addArg(asStringRef(cast<MDNode>(ArgNode)->getOperand(0)));
  }
