...
void AArch64CallLowering::saveVarArgRegisters(
  ...
  unsigned NumVariadicGPRArgRegs = NumGPRArgRegs - FirstVariadicGPR + 1;
  ...
  if (GPRSaveSize != 0) {
    ...
    for (unsigned i = FirstVariadicGPR; i < NumGPRArgRegs; ++i) {
      ...
      FIN = MIRBuilder.buildPtrAdd(
          MRI.createGenericVirtualRegister(LLT::pointer(0, 64)), FIN.getReg(0),
          Offset);
    }
  }
  ...
  if (Subtarget.hasFPARMv8() && !IsWin64) {
    ...
    if (FPRSaveSize != 0) {
      ...
      auto FIN = MIRBuilder.buildFrameIndex(LLT::pointer(0, 64), FPRIdx);
      auto Offset = MIRBuilder.buildConstant(
          MRI.createGenericVirtualRegister(LLT::scalar(64)), 16);
      ...
      for (unsigned i = FirstVariadicFPR; i < NumFPRArgRegs; ++i) {
        ...
        FIN = MIRBuilder.buildPtrAdd(
            MRI.createGenericVirtualRegister(LLT::pointer(0, 64)),
            FIN.getReg(0), Offset);
      }
    }
